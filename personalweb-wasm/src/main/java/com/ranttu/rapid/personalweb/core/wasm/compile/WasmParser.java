...
public class WasmParser {
     ...
     */
    private void doParse(Module.ModuleBuilder moduleBuilder, InputStream inputStream) {
        try {
            try {
     ...
     * parse a section
     */
    private void parseSection(Module.ModuleBuilder moduleBuilder, WasmSourceStream ins) {
    ...
    private Instruction[] readInstructions(WasmSourceStream ins) {
        var exprs = new ArrayList<Instruction>();
        ...
        return exprs.toArray(new Instruction[0]);
    }
    ...
    private Instruction parseInstruction(WasmSourceStream ins) {
        var opcode = ins.next();
        var instBuilder = Instruction.builder().opcode(opcode);
        ...
        switch (opcode) {
            ...
            case OP_LOOP: {
                ...
                return instBuilder
                    .blockInstructions(insts.toArray(new Instruction[0])).build();
            }
            //~~~ if instruction
            case OP_IF: {
                instBuilder.blockType(parseBlockType(ins));
                ...
                instBuilder.blockInstructions(insts.toArray(new Instruction[0]));
                insts.clear();
                ...
                return instBuilder
                    .elseInstructions(insts.toArray(new Instruction[0])).build();
            }
            //~~~ labeled instruction
            case OP_BR:
            case OP_BRIF: {
